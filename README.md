      ...
      /features
        /auth
          ...
          /api
            ...
            authApi.ts      // 한 개의 API 엔드포인트를 담당하는 파일 (예시)
        ...
        /dashboard
          ...
          /api
            index.ts
            dashboardApi.ts // 한 개의 API 엔드포인트를 담당하는 파일 (예시)
...
- `api/`: HTTP 클라이언트 래퍼, fetch/axios 호출, API 엔드포인트 정의  
  - **규칙:** 한 개의 API 엔드포인트당 하나의 파일로 작성  
  - 예: `authApi.ts`, `dashboardApi.ts`, `login.post.ts`, `getDashboardSummary.get.ts` 등
...
- 모든 API 파일 상단에는 JSDoc 스타일 주석으로 아래 내용을 반드시 문서화한다. 이 주석은 BE에게 그대로 전달 가능한 API 명세로 사용한다.
  - API 용도(한 줄 요약)
  - HTTP 메서드 및 URL (예: `POST /api/auth/login`)
  - 요청 타입
    - path params 타입
    - query params 타입
    - request body 타입 (필수/옵션 필드 구분)
  - 응답 타입
    - 성공 응답 타입 (status code, payload 구조)
    - 에러 응답 타입 (주요 에러 코드 및 메시지)
  - 특이사항 / 제약
    - 인증/인가 필요 여부
    - 페이징/정렬 규칙
    - 성능/제한 사항(예: 최대 row 수 등)
- FE ↔ BE 협업 시, 해당 JSDoc 블록을 그대로 복사하여 BE 설계 문서 또는 공유 문서에 붙여넣어 API 스펙으로 사용한다.
